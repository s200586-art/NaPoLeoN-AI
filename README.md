...
- `FITBIT_SLEEP_GOAL_MINUTES` — цель сна в минутах для прогресса (по умолчанию `480`)
...
- Дополнительно поддерживается:
  - одиночный диалог (не массив) для ChatGPT/Gemini/Claude;
  - JSON с плоским массивом сообщений (импортируется как один чат);
  - автоматическое удаление дублей диалогов при импорте.

## Мультиагент
- Во вкладке `Агенты` можно задать один промпт и запустить все активные модели параллельно.
- Доступен запуск как всех агентов сразу, так и каждого отдельно.
- Для каждого агента отображаются статусы (`Думает` / `В работе` / `Готово` / `Ошибка`) и текст ответа.
- Текущий состав: `Kimi`, `MiniMax`, `GLM (planned)`.

## Fitbit в дашборде
- Добавлены блоки `цели и прогресс` и `динамика за 7 дней`.
- Показываются:
  - прогресс по шагам (сегодня / цель шагов Fitbit),
  - прогресс по сну (сегодня / `FITBIT_SLEEP_GOAL_MINUTES`),
  - недельный график шагов, среднее, лучший день и число дней достижения цели.
...
- У каждой карточки сохраняется история событий:
  - создание,
  - смена статуса,
  - перенос в чат,
  - экспорт в проект.
- Экспорт выбранных карточек в Google Drive проект:
  - endpoint: `POST /api/projects/drive/export-share`,
  - в выбранной папке создаётся markdown-файл `share-inbox-*.md`,
  - после экспорта в историю карточки пишется событие `exported_to_project`.
